...
config = {
    "alpha_vantage": {
        "key": "6P03OIQGX3TR7PH1", # Claim your free API key here: https://www.alphavantage.co/support/#api-key
        "symbol": "IBM",
        "outputsize": "full",
...
# plot
def plot_data(data_date, data_close_price, num_data_points, display_date_range, config):
    fig = figure(figsize=(25, 5), dpi=80)
    fig.patch.set_facecolor((1.0, 1.0, 1.0))
    plt.plot(data_date, data_close_price, color=config["plots"]["color_actual"])
    xticks = [data_date[i] if ((i%config["plots"]["xticks_interval"] == 0 and
                                (num_data_points-i) > config["plots"]["xticks_interval"]) or i == num_data_points-1)
...
def add_to_encoded_list(current_fig):
    canvas = FigureCanvas(current_fig)
...
# split dataset
def split_dataset(data_x, data_y, config):
    split_index = int(data_y.shape[0]*config["data"]["train_split_size"])
    data_x_train = data_x[:split_index]
    data_x_val = data_x[split_index:]
    data_y_train = data_y[:split_index]
    data_y_val = data_y[split_index:]
    return data_x_train, data_x_val, data_y_train, data_y_val, split_index


# prepare data for plotting
def prepare_data_for_plotting(data_y_train, data_y_val, split_index, num_data_points, scaler):
...
class TimeSeriesDataset(Dataset):
    def __init__(self, x, y):
        x = np.expand_dims(x, 2)  # in our case, we have only 1 feature, so we need to convert `x` into [batch,
...
def run_epoch(dataloader, is_training=False, model=None, criterion=None, optimizer=None, scheduler=None):
    epoch_loss = 0
...
machine_learning = Blueprint('machine-learning', __name__)


@machine_learning.route('/machine-learning', methods=['GET', 'POST'])
@login_required
def run_machine_learning_prediction():
    if request.method == 'GET':
        ...
        plt.xticks(x, xticks, rotation='vertical')
        plt.title(
            "Daily close prices for " + config["alpha_vantage"]["symbol"] + " - showing training and validation data")
        #    plt.grid(b=None, which='major', axis='y', linestyle='--')
        plt.legend()
        #plt.show()
        add_to_encoded_list(fig)
        ...
        for epoch in range(config["training"]["num_epoch"]):
            loss_train, lr_train = run_epoch(train_dataloader, is_training=True, model=model, criterion=criterion,
                                             optimizer=optimizer, scheduler=scheduler)
            loss_val, lr_val = run_epoch(val_dataloader, model=model, criterion=criterion, optimizer=optimizer,
                                         scheduler=scheduler)
            scheduler.step()
        ...
        fig = figure(figsize=(25, 5), dpi=80)
        fig.patch.set_facecolor((1.0, 1.0, 1.0))
        plt.plot(to_plot_data_date, to_plot_data_y_val_subset, label="Actual prices",
...
@login_required
def charts():
    return render_template('machine-learning-charts.html', user=current_user)
